-- 015: pasar las banderas al formato plano de flagcdn
--
-- La 014 dejó la columna en el formato WxH, que flagcdn sirve con efecto de
-- tela ondeando. La app usa el formato w{N}, que devuelve la bandera plana y
-- rectangular como el proveedor anterior. Se normaliza la columna al mismo
-- formato para que opponent_flag (schema.sql:168) devuelva lo mismo que ve
-- el cliente.
--
-- El ancho w160 es el que TeamFlag pide en su tamaño más grande.

UPDATE teams AS t
SET flag = 'https://flagcdn.com/w160/' || v.code || '.png'
FROM (VALUES
  ...
  ('bot','bw'),('swa','sz'),('lby','ly'),('sud','sd'),('jpn','jp'),
  ('kor','kr'),('irn','ir'),('aus','au'),('sau','sa'),('qat','qa'),
  ('uae','ae'),('irq','iq'),('chn','cn'),('uzb','uz'),('tha','th'),
  ('vie','vn'),('omn','om'),('jor','jo'),('bhr','bh'),('syr','sy'),
  ('lbn','lb'),('pal','ps'),('kuw','kw'),('tkm','tm'),('kgz','kg'),
  ('tjk','tj'),('afg','af'),('ind','in'),('mya','mm'),('phi','ph'),
  ('idn','id'),('mas','my'),('sin','sg'),('ban','bd'),('prk','kp'),
  ('hkg','hk'),('yem','ye'),('pak','pk'),('sri','lk'),('mdv','mv'),
  ('nep','np'),('bhu','bt'),('cam','kh'),('lao','la'),('tls','tl'),
  ('bru','bn'),('mac','mo'),('mon','mn'),('tpe','tw'),('gum','gu'),
  ('nzl','nz'),('png','pg'),('fij','fj'),('ncl','nc'),('tah','pf'),
  ('slb','sb'),('van','vu'),('cok','ck'),('sam','ws'),('ton','to')
) AS v(id, code)
WHERE t.id = v.id AND t.flag IS DISTINCT FROM 'https://flagcdn.com/w160/' || v.code || '.png';
